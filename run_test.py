...
# input_path = 'data/pose2d_cut_testtt_no_interpolation_20_09'
args = parse_args()
print(args)
...
if args.dataset == 'h36m':
    ...
    dataset = CustomDataset('data_mul/data_2d_' + args.dataset + '_' + args.keypoints + '.npz')
    # /home/lin/workspace/detectron2/configs/COCO-Keypoints/pose2d_cut_testtt_no_interpolation_20_09.npz
    # dataset = CustomDataset(input_path)
else:
...
keypoints = np.load('data_mul/data_2d_' + args.dataset + '_' + args.keypoints + '.npz', allow_pickle=True)
...
if not args.render:
    ...
    args.viz_subject = list(keypoints.keys())[0]
...
if args.render:
    ...
    #add for loop for multi-person
    num_person = input_keypoints.shape[1]
    input_keypoint = input_keypoints
    predicted_multi_input_keypoints = []
    multi_prediction = []
    if model_traj is None and ground_truth is None:
        for i in range(num_person):
            input_keypoints = input_keypoint[:,i,:,:]
            ...
            predicted_multi_input_keypoints.append(input_keypoints)
        multi_prediction = np.array(multi_prediction, dtype=np.float32)
        predicted_multi_input_keypoints = np.array(predicted_multi_input_keypoints, dtype=np.float32)
    ...
    if args.viz_export is not None:
        ...
        np.save(args.viz_export, multi_prediction)
        # only for testing
        np.save(args.viz_export + '_predictions', multi_prediction)
        np.save(args.viz_export + '_keypoints', predicted_multi_input_keypoints)
        ...
        if ground_truth is not None:
            ...
            for subject in dataset.cameras():
                if 'orientation' in dataset.cameras()[subject][args.viz_camera]:
                    ...
                    break
            for i in range(len(multi_prediction)):
                multi_prediction[i] = camera_to_world(multi_prediction[i], R=rot, t=0)
                # We don't have the trajectory, but at least we can rebase the height
                multi_prediction[i, :, :, 2] -= np.min(multi_prediction[i, :, :, 2])
        # only for testing
        np.save(args.viz_export + '_after_camera_to_world', multi_prediction)
        anim_output = {'Reconstruction': multi_prediction}
        if ground_truth is not None and not args.viz_no_ground_truth:
            anim_output['Ground truth'] = ground_truth

        predicted_multi_input_keypoints = image_coordinates(predicted_multi_input_keypoints[..., :2], w=cam['res_w'], h=cam['res_h'])
        # only for testing
        np.save(args.viz_export + '_after_image_coordinates', predicted_multi_input_keypoints)
        from common.visualization import multi_person_render_animation
